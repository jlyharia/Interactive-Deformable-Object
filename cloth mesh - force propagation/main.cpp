...
void ForcePropaganda(const int width, vector< vector<int>> &NeighborVert)
{
    vout(NeighborVert);

    vector< vector<float>> VertForce(width * width, vector<float> (3));
    //store force applied on each vertex
    //first index -> mesh index, second index -> force

    auto del_t = 0.5f;

    for(auto t=0; t<2; ++t)//iteration times
    {
...
int main()
{
    ...
    ForcePropaganda(width, NeighborVert);
